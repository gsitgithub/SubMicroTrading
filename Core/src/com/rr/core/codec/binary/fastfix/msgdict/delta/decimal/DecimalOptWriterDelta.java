...
public final class DecimalOptWriterDelta extends DeltaFieldWriter {

    private final double         _init;
    private final FastFixDecimal _prevDecimal = new FastFixDecimal();
    
    public DecimalOptWriterDelta( String name, int id, String init ) {
        this( name, id, FieldUtils.parseDouble(init) );
    }

    public DecimalOptWriterDelta( String name, int id ) {
        this( name, id, 0 );
    }

    public DecimalOptWriterDelta( String name, int id, double init ) {
        super( name, id, true );
        _init = init;
        reset();
    }
    ...
    public void write( final FastFixBuilder encoder, final double value ) {
        ...
        if ( value != Constants.UNSET_DOUBLE ) {
            if ( ! _prevDecimal.isNull() ) {
                ...
                encoder.encodeOptionalInt( expDelta );
