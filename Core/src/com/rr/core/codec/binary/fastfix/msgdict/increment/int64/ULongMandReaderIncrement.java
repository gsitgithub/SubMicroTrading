...
public final class ULongMandReaderIncrement extends IncrementFieldReader implements FixFieldReader<LongFieldValWrapper> {

    private final long _init;
    private       long _previous;
    
    public ULongMandReaderIncrement( String name, int id, String init ) {
        this( name, id, FieldUtils.parseLong(init) );
    }

    public ULongMandReaderIncrement( String name, int id ) {
        this( name, id, Constants.UNSET_LONG );
    }
    
    public ULongMandReaderIncrement( String name, int id, long init ) {
        super( name, id, false );
    ...
    public long read( final FastFixDecodeBuilder decoder, final PresenceMapReader mapReader ) {
        ...
        if ( mapReader.isNextFieldPresent() ) {
            final long val = decoder.decodeMandULong();
            _previous = val;
            return val;
        } 
