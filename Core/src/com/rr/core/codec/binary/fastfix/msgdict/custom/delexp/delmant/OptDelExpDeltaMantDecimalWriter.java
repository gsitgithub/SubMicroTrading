 ...
 * @author Richard Rose
 */
public class OptDelExpDeltaMantDecimalWriter extends CustomDecimalFieldWriter {
    ...
    private final IntOptWriterDelta     _exp;
    private final LongMandWriterDelta   _mant;
    
    public OptDelExpDeltaMantDecimalWriter( ComponentFactory cf, String name, int id, String initExp, String initMant ) {
        this( cf, name, id, FieldUtils.parseInt(initExp), FieldUtils.parseLong(initMant) );
    }
    
    @SuppressWarnings( "boxing" )
    public OptDelExpDeltaMantDecimalWriter( ComponentFactory cf, String name, int id, int initExp, long initMant ) {
        super( name, id, true );
        
        _exp  = cf.getWriter( IntOptWriterDelta.class, name + "Exp",  id, initExp );
