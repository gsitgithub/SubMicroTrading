...
public final class ULongMandWriterDelta extends DeltaFieldWriter {

    private final long _init;
    private       long _previous;
    
    public ULongMandWriterDelta( String name, int id, String init ) {
        this( name, id, FieldUtils.parseLong(init) );
    }

    public ULongMandWriterDelta( String name, int id ) {
        this( name, id, 0 );
    }
    
    public ULongMandWriterDelta( String name, int id, long init ) {
        super( name, id, false );
        _init = init;
        reset();
    }
    ...
    public void write( final FastFixBuilder encoder, final long value ) {
        if ( value != Constants.UNSET_LONG ) {
            if ( _previous != Constants.UNSET_LONG ) {
                ...
                encoder.encodeMandULongOverflow( delta, overflow );
            } else {
