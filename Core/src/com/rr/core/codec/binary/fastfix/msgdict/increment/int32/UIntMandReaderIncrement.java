...
public final class UIntMandReaderIncrement extends IncrementFieldReader implements FixFieldReader<IntFieldValWrapper> {

    private final int _init;
    private       int _previous;
    
    public UIntMandReaderIncrement( String name, int id, String init ) {
        this( name, id, FieldUtils.parseInt(init) );
    }
    
    public UIntMandReaderIncrement( String name, int id ) {
        this( name, id, Constants.UNSET_INT );
    }
    
    public UIntMandReaderIncrement( String name, int id, int init ) {
        super( name, id, false );
        _init = init;
        reset();
    }
    ...
    public int read( final FastFixDecodeBuilder decoder, final PresenceMapReader mapReader ) {
        ...
        if ( mapReader.isNextFieldPresent() ) {
            final int val = decoder.decodeMandUInt();
            _previous = val;
            return val;
        } 
