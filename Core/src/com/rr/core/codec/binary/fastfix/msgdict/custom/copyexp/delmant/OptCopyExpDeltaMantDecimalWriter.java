 ...
 * @author Richard Rose
 */
public class OptCopyExpDeltaMantDecimalWriter extends CustomDecimalFieldWriter {
    ...
    private final IntOptWriterCopy  _exp;
    private final LongMandWriterDelta   _mant;
    
    public OptCopyExpDeltaMantDecimalWriter( ComponentFactory cf, String name, int id, String initExp, String initMant ) {
        this( cf, name, id, FieldUtils.parseInt(initExp), FieldUtils.parseLong(initMant) );
    }
    
    @SuppressWarnings( "boxing" )
    public OptCopyExpDeltaMantDecimalWriter( ComponentFactory cf, String name, int id, int initExp, long initMant ) {
        super( name, id, true );
        
        _exp  = cf.getWriter( IntOptWriterCopy.class, name + "Exp",  id, initExp );
