...
public final class LongOptWriterDelta extends DeltaFieldWriter {

    private final long _init;
    private       long _previous;
    
    public LongOptWriterDelta( String name, int id, String init ) {
        this( name, id, FieldUtils.parseLong(init) );
    }

    public LongOptWriterDelta( String name, int id ) {
        this( name, id, 0 );
    }
    
    public LongOptWriterDelta( String name, int id, long init ) {
        super( name, id, true );
        _init = init;
        reset();
    }
    ...
    public void write( final FastFixBuilder encoder, final long value ) {
        if ( value != Constants.UNSET_LONG ) {
            if ( _previous != Constants.UNSET_LONG ) {
                ...
                encoder.encodeOptionalLongOverflow( delta, overflow );
            } else {
