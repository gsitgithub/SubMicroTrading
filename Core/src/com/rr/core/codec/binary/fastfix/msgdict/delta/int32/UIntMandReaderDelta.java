...
public final class UIntMandReaderDelta extends DeltaFieldReader implements FixFieldReader<IntFieldValWrapper> {

    private final int _init;
    private       int _previous;
    
    public UIntMandReaderDelta( String name, int id, String init ) {
        this( name, id, FieldUtils.parseInt(init) );
    }

    public UIntMandReaderDelta( String name, int id ) {
        this( name, id, 0 );
    }
    
    public UIntMandReaderDelta( String name, int id, int init ) {
        super( name, id, false );
    ...
    public int read( final FastFixDecodeBuilder decoder ) {
        ...
        if( _previous != Constants.UNSET_INT ) {
            final int delta = decoder.decodeMandUIntOverflow();
