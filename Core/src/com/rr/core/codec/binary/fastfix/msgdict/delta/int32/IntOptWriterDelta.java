...
public final class IntOptWriterDelta extends DeltaFieldWriter {

    private final int _init;
    private       int _previous;
    
    public IntOptWriterDelta( String name, int id, String init ) {
        this( name, id, FieldUtils.parseInt(init) );
    }
    
    public IntOptWriterDelta( String name, int id ) {
        this( name, id, 0 );
    }
    
    public IntOptWriterDelta( String name, int id, int init ) {
        super( name, id, true );
        _init = init;
        reset();
    }
    ...
    public void write( final FastFixBuilder encoder, final int value ) {
        if ( value != Constants.UNSET_INT ) {
            if ( _previous != Constants.UNSET_INT ) {
                ...
                encoder.encodeOptionalIntOverflow( delta, overflow );
            } else {
