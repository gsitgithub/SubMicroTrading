...
public final class DecimalOptReaderDelta extends DeltaFieldReader implements FixFieldReader<DoubleFieldValWrapper> {
    ...
    private       DoubleFieldDictEntry _previous;
    
    public DecimalOptReaderDelta( String name, int id, DoubleFieldDictEntry previous ) {
        super( name, id, true );
        _previous = previous;
    ...
    public double read( final FastFixDecodeBuilder decoder ) {
        ...
        if ( ! _prevDecimal.isNull() ) {
            ...
            if ( deltaExp != Constants.UNSET_INT ) {
                ...
                double val = _prevDecimal.set( newExp, newMant );    // store new values ready for next call
