...
public class SecurityDefinitionRecycler implements Recycler<SecurityDefinitionImpl> {
    ...
    public SecurityDefinitionRecycler( int recycleSize, SuperPool<SecurityDefinitionImpl> superPool ) {
        _superPool = superPool;
        _recycleSize = recycleSize;
        try {
            _root    = SecurityDefinitionImpl.class.newInstance();
        } catch( Exception e ) {
            throw new RuntimePoolingException( "Unable to create recycle root for SecurityDefinitionImpl : " + e.getMessage(), e );
        }
    }


    @Override public void recycle( SecurityDefinitionImpl obj ) {
        if ( obj.getNext() == null ) {
