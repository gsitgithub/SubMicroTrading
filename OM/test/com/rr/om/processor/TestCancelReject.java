...
public class TestCancelReject extends BaseProcessorTestCase {
    ...
    public void testCancelRejectOrdStateExpired() {
       ...
       // SYNTH MARKET CANCEL REJECTED -----------------------------------------------------------------------------
        MarketCancelRejectImpl mcxl = FixTestUtils.getMarketCancelReject( msgBuf, 
                                                                          decoder, 
                                                                          mcan.getClOrdId(), 
                                                                          mcan.getOrigClOrdId(), 
                                                                          orderId, 
        ...
        checkClientCancelReject( nos, ccxl, clOrdId, nos.getClOrdId(), mktOrderId, CAN_REJECT,  
                                 CxlRejReason.Other, CxlRejResponseTo.CancelRequest, OrdStatus.Expired );
        ...
        assertEquals( clOrdId,                    order.getClientClOrdIdChain() );
        assertEquals( mcan.getClOrdId(),          order.getClientClOrdIdChain() ); // as exchange is not using its own ids
        assertEquals( OrdStatus.Expired,          order.getLastAckedVerion().getOrdStatus() );
