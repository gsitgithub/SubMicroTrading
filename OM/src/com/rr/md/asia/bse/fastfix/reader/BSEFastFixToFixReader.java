...
import com.rr.core.codec.binary.fastfix.fulldict.ReaderFieldClassLookup;
...
public final class BSEFastFixToFixReader implements FastFixTemplateReader {
    ...
    public BSEFastFixToFixReader( MetaTemplate mt, String name, int id, byte delim ) {
        _name = name;
        _id = id;
        _mt = mt;
        _delim = delim;
    ...
    private static ReadWrapper addNormalField( MetaFieldEntry e, ComponentFactory cf, byte delim ) {
        ...
        FieldReader r = getFieldReader( e, cf, rdrClass );
    ...
    private static FieldReader getFieldReader( MetaFieldEntry e, ComponentFactory cf, Class<? extends FieldReader> rdrClass ) {
        FieldReader r = null;
        ...
        case DEFAULT:
        case NOOP:
            r = cf.getReader( rdrClass, e.getName(), e.getId(), e.getInitValue() );
            break;
        default:
            DictEntry prev = cf.getPrevFieldValWrapper( e.getName(), e.getType(), e.getInitValue() );

            r = cf.getReader( rdrClass, e.getName(), e.getId(), prev );
            break;
        }
        return r;
    }
    ...
    @Override
    public void reset() {
        // dictionary reset not at template level
    }
}
