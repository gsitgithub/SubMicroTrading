...
public class BATSMessageValidator extends BaseExchangeValidator {
    ...
    private final void validateMinQty( final int orderQty, final ReusableString err ) {
        if ( orderQty < _minQty ) delim( err ).append( INVALID_MIN_ORD_QTY ).append( _minQty ).append( Strings.QUANTITY ).append( orderQty );
        else if ( orderQty > _maxQty ) delim( err ).append( INVALID_MAX_ORD_QTY ).append( orderQty ).append( Strings.QUANTITY ).append( _maxQty );
    }
